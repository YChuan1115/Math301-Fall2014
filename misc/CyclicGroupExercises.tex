...
\item If $gh=hg$ in a group and if both $\order{g}$ and $o(h)$ are
finite, show that $o(gh)$ is finite.
\item In the group $\GL(2,\Z)$, show that $g=\begin{bmatrix}
  0 & -1 \\
  1 & 0
\end{bmatrix}$ has order 4, $h= \begin{bmatrix}
  0 & 1 \\
  -1 & -1
\end{bmatrix}$ has order 3, but the product $gh$ has infinite
order.  Does this example contradict the statement of Part (a)?
\end{enumerate}
...
\end{enumerate}
\end{exs}
\begin{thm}    Let $g$ be an element of a group $G$.  Then there are
...
(a)  If $n|k$ then $k=qn$ for some $q\in n$.  Then
...
$g^n=1$, and it is denoted $\order{g}=n$.  If there is no such positive
integer, then we say that $g$ has {\bf infinite order}, denoted
$\order{g}=\infty$.  By Theorem 4, the concept of order of an element
...
$o(t)=|\langle g\rangle|$.
\end{cor}
\begin{proof}  This is immediate from Theorem 4, Part (c).
\end{proof}
...
divides $o(g^k)$ by Theorem 4 Part (a).  Now suppose that
$(g^k)^r=1$.  Then $g^{kr} = 1$, so by Theorem 3 Part (a), $n\mid
...
\begin{proof}  By Theorem 6,  if $m=\gcd (k, n)$, then $o(g^k)=n/m$.  But
...
question is completely answered by Theorem 10.  Theorem 9 is a
preliminary, but important, result.

\begin{thm}   Every subgroup of a cyclic group is cyclic.
\end{thm}
...
\begin{enumerate}
\item  If $H$ is any subgroup of $G$, then $H=\langle g^d\rangle$ for some $d|n$.
\item  If $H$ is any subgroup of $G$ with $|H|=k$, then $k|n$.
\item  If $k\mid n$, then $\langle g^{n/k}\rangle$ is the unique subgroup
of $G$ of order $k$.
\end{enumerate}
\end{thm}
...
$d=\gcd(m, n)$.  Since $d\mid n$ it is sufficient to show that
$H=\langle g^d\rangle$.  But $d|m$ also, so $m=qd$.  Then
...
\item By Part (a), $H=\langle g^d\rangle$ where $d\mid n$.  Then $k=|H|=
n/d$ so $k|n$.

\item Suppose that $K$ is any subgroup of $G$ of order $k$.  By
Part (a), let $K=\langle g^m\rangle$ where $m\mid n$.  Then
Theorem 6 gives $k=|K| = |g^m| = n/m$.  Hence $m=n/k$, so
$K=\langle g^{n/k}\rangle$.  This proves (c).
\end{enumerate}
\end{proof}

\begin{rem} Part (b) of Theorem 10 is actually true for {\it any} finite
group $G$, whether or not it is cyclic.  This result is Lagrange's
Theorem (Theorem 5.2.3, Page 219 of your text).
\end{rem}
...
g^2\rangle , \quad \langle g\rangle = G.$$ Note that $\langle
g^m\rangle \subseteq \langle g^k\rangle$ if and only if $k\mid m$.
...
\begin{array}{cccc}
&&\Rnode{a}{G}& \\[1cm]
&\Rnode{b}{\langle g^2\rangle}&&\Rnode{c}{\langle g^3\rangle}
\\[1cm]
\Rnode{d}{\langle g^4\rangle}&&\Rnode{e}{\langle g^6\rangle}&\\[1cm]
&\Rnode{f}{\langle 1\rangle}&&\\[1cm]
\end{array}
\psset{nodesep=5pt} \ncline{f}{d} \ncline{f}{e} \ncline{d}{b}
\ncline{e}{b} \ncline{e}{c} \ncline {b}{a} \ncline {c}{a}
$$
...
$$\begin{matrix}&&&&G\\
&&&\nearrow&&\nwarrow\\ &&\langle g^2\rangle&&&&\langle g^3\rangle\\
&\nearrow&&\nwarrow&&\nearrow\\ \langle g^4\rangle &&&&\langle
g^6\rangle\\ &\nwarrow&&\nearrow\\ &&\langle 1\rangle\\
\end{matrix}$$
